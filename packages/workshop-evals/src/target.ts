...
// Runtimes whose shutdown failed. Such a workerd may still run model-authored code, and its requests
// route through this process's fetch, so the egress filter must outlive it (see defineTaskEval).
let leakedRuntimes = 0;

/** How many eval runtimes this process could not confirm stopped. */
export function runtimesStillRunning(): number {
  return leakedRuntimes;
}
