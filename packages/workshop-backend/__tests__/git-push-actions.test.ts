...
function installPackReceiver(
    ...
    props: { packAction?: number; stoppedAt?: number; throwAfterBuild?: boolean } = {}) {
...
describe("push authorization through the Overseer chokepoints", () => {
  ...
  it("keeps the pack callback optional and supplies it only for declared pushes", async () => {
    await inOverseer("batch-pack-optional", async impl => {
      const receiver = installPackReceiver(impl, GATEKEEPER);
      expect(await receiver.applyActionsThrough(99, [])).toStrictEqual({});

      await impl.submitAction(GATEKEEPER, 1, {
        title: "Ordinary action",
        description: "Does not push Git objects.",
        implementsRevert: true,
      }, { from: "user" });
      await impl.applyDecidedActions(GATEKEEPER, { action: 1, resolvedBy: USER });
      expect(actionRecord(impl, GATEKEEPER, 1).state).toBe("approved");

      await impl.submitAction(GATEKEEPER, 2, pushDescription([]), { from: "user" });
      await impl.applyDecidedActions(GATEKEEPER, { action: 2, resolvedBy: USER });
      expect(actionRecord(impl, GATEKEEPER, 2).state).toBe("approved");
    });

    await inOverseer("batch-pack-unavailable", async impl => {
      const { head } = await seedPushableHistory(impl);
      const receiver = installPackReceiver(impl, GATEKEEPER, { packAction: 41 });
      await impl.submitAction(GATEKEEPER, 41, pushDescription([head]), { from: "user" });
      const record = actionRecord(impl, GATEKEEPER, 41);

      const result = await receiver.applyActionsThrough(41, []);

      expect(result.stopped?.at).toBe(41);
      expect(getGitPackErrorCode(result.stopped?.reason)).toBe(GIT_PACK_ERROR_CODES.unavailable);
      expect(actionRecord(impl, GATEKEEPER, 41).state).toBe("pending");
      expect(marksOf(impl, record.id)).toStrictEqual([head]);
      expect(impl.storage.gitObjectMetadata.get(head)!.onRemote).not.toContain(GATEKEEPER);
    });
  });

  it("builds the selected push when a different action sets the frontier", async () => {
    await inOverseer("batch-pack-frontier", async impl => {
      ...
      try {
        await impl.submitAction(GATEKEEPER, 41, {
          ...pushDescription([head]),
          autoApprovable: true,
          actionKind: kind,
        }, { from: "user" });
        ...
        const frontier = actionRecord(impl, GATEKEEPER, 99);
        expect([push.id, frontier.id]).not.toContain(41);
        expect([push.id, frontier.id]).not.toContain(99);

        await impl.applyDecidedActions(GATEKEEPER, { action: 99, resolvedBy: USER });
        ...
        let caught: unknown;
        try {
          await receiver.buildRetained(41);
        } catch (error) {
          caught = error;
        }
  ...
  it("authorizes exact local push selectors without workspace-ID collisions", async () => {
    await inOverseer("batch-pack-selectors", async impl => {
      ...
      const vetoHistory = await seedPushableHistory(impl, GATEKEEPER, " vetoed");
      await impl.submitAction(
          GATEKEEPER, foreignId + 2, pushDescription([vetoHistory.head]), { from: "user" });
      const vetoed = actionRecord(impl, GATEKEEPER, foreignId + 2);
      impl.storage.transaction(() => {
        vetoed.state = "rejected";
        impl.gitCache.clearPushMarks(vetoed.id);
        impl.storage.actions.put(vetoed);
      });
      ...
      try {
        ...
        const laterHistory = await seedPushableHistory(impl, GATEKEEPER, " later");
        await impl.submitAction(
            GATEKEEPER, foreignId + 4, pushDescription([laterHistory.head]), { from: "user" });
        const later = actionRecord(impl, GATEKEEPER, foreignId + 4);

        for (const selector of [own.id, 999_999, nonPush.action, vetoed.action, later.action]) {
  ...
  it("keeps one push's shared-object grant after rejecting a sibling push", async () => {
    await inOverseer("batch-pack-shared-grant", async impl => {
      impl.storage.gatekeepers.put({ id: GATEKEEPER, class: {} });
      const { head: shared } = await seedPushableHistory(impl, GATEKEEPER, " shared");
      const tree = await impl.gitCache.putFromGatekeeper(
          GATEKEEPER, "tree", new Uint8Array(0));
      const childA = await storeLocal(
          impl, "commit", commitPayload(tree, [shared], "child A"));
      const childB = await storeLocal(
          impl, "commit", commitPayload(tree, [shared], "child B"));

      await impl.submitAction(GATEKEEPER, 51, pushDescription([childA]), { from: "user" });
      await impl.submitAction(GATEKEEPER, 52, pushDescription([childB]), { from: "user" });
      const actionA = actionRecord(impl, GATEKEEPER, 51);
      const actionB = actionRecord(impl, GATEKEEPER, 52);
      const builder = new GitPackBuilderImpl(
          impl.gitCache, impl.storage, GATEKEEPER, [actionA, actionB]);

      try {
        impl.storage.transaction(() => {
          actionB.state = "rejected";
          impl.gitCache.clearPushMarks(actionB.id);
          impl.storage.actions.put(actionB);
        });

        const objects = await decodePackBytes(
            await collect(await builder.buildPack(51)), { maxObjectSize: 1 << 20 });
        const oids = await Promise.all(
            objects.map(object => gitObjectOid(object.type, object.payload)));
        expect(oids.toSorted()).toStrictEqual([childA, shared].toSorted());
        expect(oids).not.toContain(childB);
        expect(marksOf(impl, actionA.id).toSorted()).toStrictEqual([childA, shared].toSorted());
        expect(marksOf(impl, actionB.id)).toStrictEqual([]);
        expect(impl.storage.gitObjectMetadata.get(childA)!.onRemote)
